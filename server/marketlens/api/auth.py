...
지금까지 이 서버는 노트북에서만 떴고 라우트 스물다섯 개가 전부 열려 있었다. 폰에서
쓰려고 공개 주소에 올리는 순간 그건 **누구나 쓰는 서버**가 된다 — 시세만 나가는 게
...
- `MARKET_LENS_TOKEN` 이 **없으면 잠그지 않는다.** 집에서 개발할 때 매번 토큰을 넣게
  하면 그 불편이 결국 토큰을 지우게 만든다. 배포에서만 켠다.
- 있으면 `/api/*` 와 `/ws` 전부에 요구한다.
- **`/api/health` 만 연다.** 배포판(Fly)이 살아있는지 볼 수 있어야 하고, 여기서 나가는
  건 "떠 있다"와 지표 개수뿐이다.
