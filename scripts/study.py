...
#
# **재서 정했고, 한 번 고쳤다.** `scripts/overfitcheck.py` 가 결과를 덩어리째 섞어
# 같은 탐색을 200번 돌린다. 거기서 나오는 이득은 전부 운이니 그 분포가 곧 문턱이다.
#
# 처음엔 감으로 2%p. 재 보니 귀무 95% 가 3.8%p 라 4%p 로 올렸다. 그런데 그때 쓴
# **덩어리 크기 200 을 손으로 골랐다** — 근거가 없었다. `arch` 의 최적 블록 길이
# (Politis & White 2004)로 바꾸니 실제 계열은 405판씩 뭉쳐 있었고 귀무 95% 가
# **6.3%p** 로 넓어졌다. 덩어리를 작게 잡으면 귀무 세계가 실제보다 깨끗해져
# 문턱이 낮게 잡힌다.
#
# 그래서 7%p 다. 높거나 낮아 보이면 **`overfitcheck.py` 를 돌려 보고** 고칠 것 —
# 감으로 움직이지 말 것.
MIN_HOLDOUT_GAIN = 0.07
