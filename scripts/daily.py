...
- **승격 문턱이 도전 횟수를 본다**(`promote_bar`). 고정 마진 하나로는 못 막는다 —
  같은 자리에 여러 번 도전할수록 잡음만으로 이기는 일이 흔해지기 때문이다.
  잡음의 폭은 가정하지 않고 로그에서 잰다(`null_std`).
...
def write_summary(state: dict[str, Record], entries: list[dict]) -> None:
    ...
    lines = [
        ...
        f"(최소 {PROMOTE_MARGIN:+.3f}). 문턱을 정한 근거와 잰 방법은",
