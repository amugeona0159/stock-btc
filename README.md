...
- **아침 매수 추천** — 매일 07:30 에 한 번 뽑아 그날은 고정된다. **국내주식·해외주식·
  코인 세 묶음**을 한 번에 내고, 묶음마다 기대 수익률 순으로 사라 3 + 사지 말 것 3.
  화면이 시장을 고르게 하지 않는다 — 차트가 BTC 에 서 있다고 코인만 보이면 안 된다.
  지평이 지나면 실제와 맞춰 **그 묶음 후보 전체 평균과 견준 성적**을 쌓는다 —
  이기든 지든 그대로 남는다. **되돌려 채점한 결과는 아래 '한계' 에 그대로 적었다.**
  종목은 **한글 이름과 원화**로 나온다(`솔라나 SOL · ₩144,400`). 코인 가격은 환율을
  곱한 값이 아니라 **업비트 원화 마켓의 실거래가**다 — 환산가는 김치 프리미엄만큼
  어긋나 그 값에 살 수가 없다. 그래서 원화 마켓이 없는 코인은 후보에서 뺀다
...
.venv/Scripts/python scripts/backfill.py --summary   # 추천을 되돌려 채점한 성적
...
소수점까지 믿을 숫자는 아니다. 이 표는 **한 종목만으로 학습하던 시절**의 값이다
(`--peers 0`). 지금은 동료 종목까지 모아 학습하므로 다시 재면 달라진다.
...
- **추천이 후보 전체 평균을 이긴다는 증거가 없다.** 다섯 시장을 과거로 되돌려
  채점해 보니 holdout 열다섯 칸이 전부 음수였고, 덩어리 부트스트랩으로 재면 서른 칸 중
  95% 구간이 0 을 벗어난 것이 하나도 없다. 국내주식이 한때 혼자 양수였는데 origin 을
  60→200 으로 늘리자 세 칸이 전부 부호를 바꿨다 — 표본을 어디서 자르느냐가 답을 정하고
  있었다는 뜻이다. 자세한 것은 [docs/RECOMMEND.md](docs/RECOMMEND.md).
  **잘 맞는 것은 폭이지 순위가 아니다.**
- **배포하지 않는다.** Fly.io 설정이 있었지만 한 번도 배포된 적이 없어 지웠다.
  이 도구는 PC 한 대에서 돈다 — 폰 앱(PWA)과 웹푸시 코드는 남아 있지만 HTTPS 주소가
  없어 사실상 잠들어 있다
